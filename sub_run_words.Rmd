...
library(ggdist)
library(stringr)
raw = rawmr %>% filter(response_time>0.100) %>% dplyr::select(response_time, word, gender, sub, run,year,month,day, con,trial_type,trial_type1)
...
raw %>% filter(str_detect(trial_type, "image") == F)
